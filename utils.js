...
*/
    handleReminder : message => {
        ...
        if (!time[0] || !time[1] || isNaN(time[0]) || isNaN(time[1]) || time[0] < 0 || time[0] > 23 || time[1] < 0 || time[1] > 59) {
            message.channel.send("Erreur : la date est mal formatée !");
            return;
        } else {
            let content = tools.patchArgs(args, 2);
            if (content !== "") {
                let reminder_obj = {
                    'message'   : message,
                    'hours'     : time[0],
    ...
    checkReminder : () => {
        ...
        for (let i = 0; i < reminder_tab.length; ++i) {
            if (reminder_tab[i].hours == currentTime.getHours() + 1 && reminder_tab[i].minutes == currentTime.getMinutes()) {
